...
MixingBrush::MixingBrush(const MixingProperties* properties, KoColorTransformation* transformation)
{
...
void MixingBrush::paint(KisPaintDeviceSP dev, qreal x, qreal y, const KoColor &color)
{
    ...
    for (int by = -m_properties->radius; by <= m_properties->radius; by++) {
        ...
        for (int bx = -m_properties->radius; bx <= m_properties->radius; bx++) {
            // let's call that noise from ground to mixing :)
